...
var parentGrpA = (await client.PostAsJsonAsync(url, new { id = "0", api_key = key, method = "create_repo_group", args = new { group_name = parentNameA, } })).Content.ReadFromJsonAsync<JsonElement>();
var parentNameB = $"parent_{now:HHmm_ss}_B";
var parentGrpB = (await client.PostAsJsonAsync(url, new { id = "0", api_key = key, method = "create_repo_group", args = new { group_name = parentNameB, } })).Content.ReadFromJsonAsync<JsonElement>();
var subName = $"sub_{now:HHmm_ss}";
var subGrp = (await client.PostAsJsonAsync(url, new { id = "0", api_key = key, method = "create_repo_group", args = new { group_name = subName, parent = parentNameA, } })).Content.ReadFromJsonAsync<JsonElement>();
...
var parameter = new
{
    ...
    args = new
    {
        repogroupid = $"{parentNameA}/{subName}",
        parent = parentNameB,
    }
};
